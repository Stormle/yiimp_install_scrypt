...
output ""
    ...
    output "Creating frontend, panel and stratum users."
    sudo mkdir -p /home/frontend
    sudo chmod -R 770 /home/frontend
    sudo useradd frontend -s /bin/bash
    sudo usermod -p '!' frontend
    sudo chown -R frontend.frontend /home/frontend
    sudo mkdir -p /home/panel
    sudo chmod -R 770 /home/panel
    sudo useradd panel -s /bin/bash
    sudo usermod -p '!' panel
    sudo chown -R panel.panel /home/panel
    sudo mkdir -p /home/stratum
...
' | sudo -E tee /etc/nginx/blockuseragents.rules >/dev/null 2>&1
    ...
    echo "panel:$basicauthpanel" | sudo tee /etc/nginx/pma_pass_panel
    ...
    sudo ln -s /home/panel/yiimp/web/ /var/web
...
#fixing yiimp
    sudo sed -i "s|ROOTDIR=/data/yiimp|ROOTDIR=/home/panel/yiimp|g" /bin/yiimp
    #fixing run.sh
...
sudo chmod +x /var/stratum/config/run.sh
    ...
    # adding user to group, creating dir structure, setting permissions
      ...
      sudo mkdir -p /var/www/panel.$server_name/html  
      sudo cp /etc/php/7.0/fpm/pool.d/www.conf /etc/php/7.0/fpm/pool.d/frontend.conf
      sudo cp /etc/php/7.0/fpm/pool.d/www.conf /etc/php/7.0/fpm/pool.d/panel.conf
      sudo sed -i 's/\[www\]/\[frontend\]/' /etc/php/7.0/fpm/pool.d/frontend.conf
      sudo sed -i 's/^user = www-data/user = frontend/' /etc/php/7.0/fpm/pool.d/frontend.conf
      sudo sed -i 's/^group = www-data/group = frontend/' /etc/php/7.0/fpm/pool.d/frontend.conf
      sudo sed -i 's/php7\.0-fpm\.sock/php7\.0-fpm-frontend\.sock/' /etc/php/7.0/fpm/pool.d/frontend.conf
      sudo sed -i 's/\[www\]/\[panel\]/' /etc/php/7.0/fpm/pool.d/panel.conf
...
echo 'include /etc/nginx/blockuseragents.rules;
	...
	if ($blockedagent) {
        ...
        listen 80;
        listen [::]:80;
        server_name panel.'"${server_name}"';
        root "/var/www/panel.'"${server_name}"'/html/web";
        ...
        access_log off;
        error_log  /var/log/nginx/panel.'"${server_name}"'.app-error.log error;
 	...
 	client_body_buffer_size  50k;
        ...
        location ~ ^/index\.php$ {
            ...
            fastcgi_pass unix:/var/run/php/php7.0-fpm-panel.sock;
...
echo 'include /etc/nginx/blockuseragents.rules;
	...
	if ($blockedagent) {
 	...
 	client_body_buffer_size  50k;
        ...
        location ~ ^/index\.php$ {
            fastcgi_split_path_info ^(.+\.php)(/.+)$;
            fastcgi_pass unix:/var/run/php/php7.0-fpm-frontend.sock;
            fastcgi_index index.php;
            include fastcgi_params;
            fastcgi_param SCRIPT_FILENAME $document_root$fastcgi_script_name;
            fastcgi_intercept_errors off;
...
sudo service nginx restart
    ...
    sudo letsencrypt certonly -a webroot --webroot-path=/home/frontend/yiimp/web --email "$EMAIL" --agree-tos -d "$server_name" -d www."$server_name"
    sudo letsencrypt certonly -a webroot --webroot-path=/home/panel/yiimp/web --email "$EMAIL" --agree-tos -d panel."$server_name"
    sudo rm /etc/nginx/sites-available/$server_name.conf
    sudo rm /etc/nginx/sites-available/panel.$server_name.conf
...
echo 'include /etc/nginx/blockuseragents.rules;
	...
	if ($blockedagent) {
    	...
    	# enforce https
        location /.well-known {
            alias /var/www/panel.'"${server_name}"'/html/web/.well-known;
        allow all;
	...
	if ($blockedagent) {
        ...
        return 444;
            ...
            access_log /var/log/nginx/panel.'"${server_name}"'.app-accress.log;
            error_log  /var/log/nginx/panel.'"${server_name}"'.app-error.log error;
 	...
 	client_body_buffer_size  50k;
        ...
        sendfile off;
            ...
            ssl_certificate /etc/letsencrypt/live/panel.'"${server_name}"'/fullchain.pem;
            ssl_certificate_key /etc/letsencrypt/live/panel.'"${server_name}"'/privkey.pem;
...
echo 'include /etc/nginx/blockuseragents.rules;
    ...
    if ($blockedagent) {
        ...
        # enforce https
        location /.well-known {
            alias /var/www/'"${server_name}"'/html/web/.well-known;
        allow all;
            default_type "text/plain";
            autoindex    on;
        }
...
sudo service php7.0-fpm reload
    ...
    Q1="GRANT SELECT, SHOW VIEW ON *.* TO 'frontend'@'localhost' IDENTIFIED BY '$password3';"
    Q2="FLUSH PRIVILEGES;"
    SQL="${Q1}${Q2}"
    sudo mysql -u root -p="" -e "$SQL"  
...
' | sudo -E tee /etc/yiimp/keys.php >/dev/null 2>&1
    ...
    output "Database 'yiimpfrontend' and users 'panel', 'stratum' and 'frontend' created with password $password, $password2 and $password3, will be saved for you"
    output ""
...
define('"'"'YAAMP_LOGS'"'"', '"'"'/home/panel/yiimp/log'"'"');
define('"'"'YAAMP_HTDOCS'"'"', '"'"'/home/panel/yiimp/web'"'"');
define('"'"'YAAMP_BIN'"'"', '"'"'/home/panel/yiimp/bin'"'"');
...
define('"'"'YAAMP_SITE_URL'"'"', '"'"'panel.'"${server_name}"''"'"');
define('"'"'YAAMP_STRATUM_URL'"'"', YAAMP_SITE_URL); // change if your stratum server is on a different host
define('"'"'YAAMP_SITE_NAME'"'"', '"'"'TheCryptoPool-panel'"'"');
...
' | sudo -E tee /home/panel/yiimp/web/serverconfig.php >/dev/null 2>&1
echo '
<?php
ini_set('"'"'date.timezone'"'"', '"'"'UTC'"'"');
define('"'"'YAAMP_LOGS'"'"', '"'"'/home/frontend/yiimp/log'"'"');
define('"'"'YAAMP_HTDOCS'"'"', '"'"'/home/frontend/yiimp/web'"'"');
define('"'"'YAAMP_BIN'"'"', '"'"'/home/frontend/yiimp/bin'"'"');
define('"'"'YAAMP_DBHOST'"'"', '"'"'localhost'"'"');
define('"'"'YAAMP_DBNAME'"'"', '"'"'yiimpfrontend'"'"');
define('"'"'YAAMP_DBUSER'"'"', '"'"'frontend'"'"');
define('"'"'YAAMP_DBPASSWORD'"'"', '"'"''"${password3}"''"'"');
define('"'"'YAAMP_PRODUCTION'"'"', true);
define('"'"'YAAMP_RENTAL'"'"', true);
define('"'"'YAAMP_LIMIT_ESTIMATE'"'"', false);
define('"'"'YAAMP_FEES_MINING'"'"', 0.5);
...
' | sudo -E tee /home/frontend/yiimp/web/serverconfig.php >/dev/null 2>&1
...
sudo find /home/panel -name '*.php' -type f -exec chmod 600 {} \;
